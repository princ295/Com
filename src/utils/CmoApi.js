...
export const apiRequest = {
  ...
  getRecordBasedOnDate: ({ category, applicantCategory, fromdate, todate, reportType} ) => {
    return axios.get(`${process.env.REACT_APP_BASE_URL}Report/GetDistrictAndHODApplication/${category}/${fromdate}/${todate}/${applicantCategory}/${reportType}`)
    .then(response => {
      let object = [];
      response.data.table.forEach((item, i) =>
        object.push({
          sno: i + 1,
          category: item.category,
          curr_Completed_Letter: item.curr_Completed_Letter,
  ...
  getRecordMainDepartmentBasedOnDate: ({ fromdate, todate}) => {
    return axios.get(`${process.env.REACT_APP_BASE_URL}Report/GetBaseDepartmentData/${fromdate}/${todate}`)
    .then(info => {
  ...
  getLetterStatus: (complaintID) => {
    return axios.get(`${process.env.REACT_APP_BASE_URL}Report/GetLetterStatus/${complaintID}`)
    .then(info => {
      return info.data.table
    })
    .catch(err => {
